...
@frappe.whitelist()
def validate_qty_in_bom_standard_deviation(bom_items, selected_item_code=None):
    """
    ...
    Returns:
    dict: A dictionary with item codes as keys and a boolean indicating if the quantity
    ...
    try:
        ...
        for item in bom_items:
            item_code = item.get('item_code')
            if selected_item_code and item_code != selected_item_code:
                continue
            ...
            validation_result[item_code] = is_valid

            # If the new quantity is outside the allowable range, print a message
            if not is_valid:
                frappe.msgprint(
                    f"Quantity for {item_code} exceeds allowable range. Allowed: {lower_bound} - {upper_bound}, Provided: {new_qty}"
                )
        ...
        if selected_item_code:
            ...
            frappe.msgprint(f"No selected item code provided or item not found.")

    except (json.JSONDecodeError, TypeError, ValueError) as e:
        frappe.msgprint(f"Error parsing JSON data: {e}")
        return {}
    ...
    if doc.stock_entry_type in ['Material Transfer for Manufacture']:
        for item in doc.items:
            ...
            if not item.serial_and_batch_bundle and not item.batch_no:
                # frappe.throw(_("Row {0}: Either (Batch Bundle or Batch No) must be provided.").format(item.idx))
                frappe.msgprint(_("Row {0}: Either ( Batch Bundle or Batch No ) must be provided.").format(item.idx),
                                alert=True)
