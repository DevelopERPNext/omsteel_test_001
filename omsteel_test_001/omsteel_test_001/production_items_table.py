...
def update_finished_item_qty_uom_piece(doc, method=None):
    ...
    update_qty_uom_a_012_detail = frappe.get_all(
        "Stock Entry",
        filters={"name": doc.name},
        fields=["update_qty_uom_a_012", ],
    )
    update_qty_uom_a_012_info = update_qty_uom_a_012_detail[0].get('update_qty_uom_a_012')
    ...
    for item in stock_entry_items:
        if item["is_finished_item"] == 1 and item["uom"] == "حبة":
            ...
            if item_details:
                tie_the_bundle_a_010 = item_details

                if item["qty"] < tie_the_bundle_a_010:
                    frappe.msgprint(
                        _(f"Skipping update for item {item['item_code']} because qty ({item['qty']}) is less than tie_the_bundle_a_010 ({tie_the_bundle_a_010})."),
                        alert=True
                    )
                    return

    # if update_qty_uom_a_012_info == 1 or update_qty_uom_a_012_info == 0:
    if update_qty_uom_a_012_info == 0:
        ...
        try:
            ...
            frappe.db.set_value("Stock Entry", doc, "update_qty_uom_a_012", 1)
...
def update_production_items_qty_uom_piece(doc, method=None):
    ...
    update_production_qty_uom_a_014_detail = frappe.get_all(
        "Stock Entry",
        filters={"name": doc.name},
        fields=["update_production_qty_uom_a_014", ],
    )
    update_production_qty_uom_a_014_info = update_production_qty_uom_a_014_detail[0].get('update_production_qty_uom_a_014')
    ...
    stock_entry_items = frappe.get_all(
        ...
        filters={"parent": doc.name},
        fields=[
            "name", "item_code", "qty", "uom", "is_scrap_item","is_finished_item",
            "basic_rate", "bom_no", "item_name", "standard_deviation_a_020",
            "t_warehouse", "stock_uom", "conversion_factor"
        ]
    )

    if update_production_qty_uom_a_014_info == 0:
        ...
        try:
            ...
            # for finished_item in stock_entry_items:
            #     if finished_item["is_finished_item"] == 1 and finished_item["uom"] == "حبة":
            #         pass
            ...
            for scrap_item in stock_entry_items:
                if scrap_item["is_scrap_item"] and scrap_item["uom"] == "حبة":
                    ...
                    tie_the_bundle_a_010 = frappe.db.get_value("Item", scrap_item["item_code"],
                                                               "tie_the_bundle_a_010")
                    if not tie_the_bundle_a_010:
                        frappe.throw(_(f"Bundle size not defined for item {scrap_item['item_code']}"))

                    if scrap_item["qty"] < tie_the_bundle_a_010:
                        frappe.msgprint(
                            _(f"Skipping update for item {scrap_item['item_code']} because qty ({scrap_item['qty']}) is less than tie_the_bundle_a_010 ({tie_the_bundle_a_010})."),
                            alert=True
                        )
                        continue
            ...
            frappe.db.set_value("Stock Entry", doc.name, "update_production_qty_uom_a_014", 1)
...
# ========================================================================
# ======================   UOM (متر) meter ========================
# ========= START Fetch Qty (Finished Item) & UOM (متر) tie_the_bundle_a_010 ===========
...
@frappe.whitelist()
def update_finished_item_qty_uom_piece_meter(doc, method=None):
    """
    Updates the Stock Entry items by splitting items into separate rows if:
    - is_finished_item is true
    - UOM is "متر"
    ...
    update_qty_uom_a_016_detail = frappe.get_all(
        "Stock Entry",
        filters={"name": doc.name},
        fields=["update_qty_uom_a_016", ],
    )
    update_qty_uom_a_016_info = update_qty_uom_a_016_detail[0].get('update_qty_uom_a_016')
    ...
    for item in stock_entry_items:
        if item["is_finished_item"] == 1 and item["uom"] == "متر":
            item_details = frappe.get_value(
                "Item",
                {"item_code": item["item_code"]},
                "tie_the_bundle_a_010"
    ...
    # if update_qty_uom_a_016_info == 1 or update_qty_uom_a_016_info == 0:
    if update_qty_uom_a_016_info == 0:
        ...
        # Loop to finished item code
        for item_a in stock_entry_items:
            # if item_a["is_finished_item"] == 1 and item_a["uom"] == "متر" and item_a["qty"] > 100:
            if item_a["is_finished_item"] == 1 and item_a["uom"] == "متر":
                item_code_is_finished_item = item_a["item_code"]
                # frappe.msgprint("OK")
                # frappe.msgprint(str(item_code_is_finished_item))
                break
        ...
        for item_aa in stock_entry_items:
            if item_aa["is_finished_item"] == 1 and item_aa["uom"] == "متر":
                total_qty = item_aa["qty"]
        ...
        try:
            ...
            frappe.db.set_value("Stock Entry", doc, "update_qty_uom_a_016", 1)
...
# =======  END Fetch Qty (Finished Item) & UOM (متر)  tie_the_bundle_a_010  ========
# ========================================================================
# ========================================================================
...
# ========================================================================
# ======================   UOM (متر) meter ========================
# ========= START Fetch Qty (Scrap Item) (Production Items Table) & UOM (متر) tie_the_bundle_a_010 ===========
...
@frappe.whitelist()
def update_production_items_qty_uom_piece_meter(doc, method=None):
    """
    Updates the Stock Entry items by splitting items into separate rows if:
    - is_scrap_item is true
    - UOM is "متر"
    ...
    update_production_qty_uom_a_018_detail = frappe.get_all(
        "Stock Entry",
        filters={"name": doc.name},
        fields=["update_production_qty_uom_a_018", ],
    )
    update_production_qty_uom_a_018_info = update_production_qty_uom_a_018_detail[0].get('update_production_qty_uom_a_018')
    ...
    if update_production_qty_uom_a_018_info == 0:
        ...
        try:
            ...
            for scrap_item in stock_entry_items:
                if scrap_item["is_scrap_item"] and scrap_item["uom"] == "متر":
                    item_code = scrap_item["item_code"]
                    qty = scrap_item["qty"]
            ...
            frappe.db.set_value("Stock Entry", doc.name, "update_production_qty_uom_a_018", 1)
...
# =======  END Fetch Qty (Scrap Item) (Production Items Table) & UOM (متر)  tie_the_bundle_a_010  ========
# ========================================================================
# ========================================================================
