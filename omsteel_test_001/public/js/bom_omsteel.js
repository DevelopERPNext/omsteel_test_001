...
function fetch_quantity_bb(frm, item_code, idx, warehouse, new_qty) {
    ...
    console.log('Fetching quantity for:', { item_code, idx, warehouse, new_qty });

    // Find the item in the items table based on item_code and idx
    let item = frm.doc.items.find(item => item.item_code === item_code && item.idx === idx);

    // Debugging: Log if item is found or not
    if (item) {
        console.log('Item found:', item);

        // Call server-side method to fetch batch quantities
        frappe.call({
            ...
            callback: function(res) {
                if (res.message) {
                    // Calculate the total quantity from the batch quantities
                    let total_qty = res.message.reduce((total, batch) => total + batch.batch_qty, 0);

//                    console.log('Total quantity from batches:', total_qty);

                    // Update the quantity for the specific item in the items table
                    frm.doc.items.forEach(item => {
                        if (item.item_code === item_code && item.idx === idx) {
                            item.qty = new_qty;
                        }
                    });

                    // Refresh the items field to reflect the updated quantity
...
//        console.log('Item not found for:', { item_code, idx });
        frappe.show_alert({
